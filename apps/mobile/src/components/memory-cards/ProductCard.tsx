...
export function ProductCard({
  ...
  return (
    <>
      {/* Product Details (Brand + Model + Price + Category highlighted) */}
      {hasProductDetails && (
        <View className="mb-6 p-4 bg-amber-50 rounded-lg border border-amber-200">
          <Text className="text-lg font-semibold text-gray-900 mb-3">Product</Text>
          {aiBrand && (
            <View className="mb-2">
              <Text className="text-sm text-gray-600">
                <Text className="font-semibold">Brand:</Text> {aiBrand}
              </Text>
            </View>
          )}
          {aiModel && (
            <View className="mb-2">
              <Text className="text-sm text-gray-600">
                <Text className="font-semibold">Model:</Text> {aiModel}
              </Text>
            </View>
          )}
          {aiPrice && (
          ...
          {aiCategory && (
            <View>
              <Text className="text-sm text-gray-600">
                <Text className="font-semibold">Category:</Text> {aiCategory}
              </Text>
            </View>
          )}
        </View>
